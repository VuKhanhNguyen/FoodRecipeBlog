...
// Public endpoint to get favorites count for a recipe
favoriteRouter.get("/count/:recipeId", async (req: Request, res: Response) => {
  try {
    const { recipeId } = req.params;
    const count = await FavoriteService.countByRecipeId(recipeId);
    return res.status(HTTP_STATUS_CODES.Ok).json({ count });
  } catch (error) {
    logger.err(error, true);
    return res.status(HTTP_STATUS_CODES.InternalServerError).json({
      error: "Lỗi server khi lấy số yêu thích. | 500",
    });
  }
});
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      return res
        ...
        .json({ error: "Lỗi server khi lấy danh sách yêu thích | 500" });
    }
  }
);
...
favoriteRouter.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      logger.err(error, true);
      return res.status(HTTP_STATUS_CODES.InternalServerError).json({
        error: "Lỗi server khi lấy danh sách yêu thích của bạn | 500",
      });
    }
  }
);
...
favoriteRouter.post(
  ...
  authenticateToken,
  async (
    req: Request<object, object, ICreateFavoriteReqBody>,
    res: Response
  ) => {
    try {
      ...
      // If it already existed, service returns existing doc — treat as OK
      return res.status(HTTP_STATUS_CODES.Ok).json({ favorite });
    } catch (error) {
      logger.err(error, true);
      if (error instanceof RouteError) {
    ...
    try {
      ...
      if (!favorite) {
        return res
          ...
          .json({ error: "Không tìm thấy yêu thích" });
      }

      if (favorite.userId.toString() !== userId && user.role !== "admin") {
        return res
          .status(HTTP_STATUS_CODES.Forbidden)
          .json({ error: "Bạn không có quyền xóa yêu thích này" });
      }

      await FavoriteService.deleteFavorite(id);
      return res
        .status(HTTP_STATUS_CODES.Ok)
        .json({ message: "Đã xóa yêu thích thành công" });
    } catch (error) {
      logger.err(error, true);
      if (error instanceof RouteError) {
        return res.status(error.status).json({ error: error.message });
